...
const MagicBento = ({
  ...
  const cardClassName = `magic-bento-card ${textAutoHide ? "magic-bento-card--text-autohide" : ""} ${
        ...
        {items.map((item) => {
          const content = (
            <a
              ...
              className="relative z-2 flex h-full flex-col justify-end"
            >
          ...
          const cardStyle: CSSProperties = { ["--glow-color" as string]: glowColor };
          ...
          return enableStars ? (
            ...
            <div key={item.title} className={cardClassName} style={cardStyle}>
              {content}
            </div>
          );
        })}
