...
type Props = {
  ...
  dpr?: number;
  fps?: number;
  /** Freezes the animation loop — e.g. the layer has faded out of view. */
  paused?: boolean;
  className?: string;
  style?: CSSProperties;
};
...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    // A shader animation is exactly the motion "reduce motion" asks us to
    // drop; render one static frame and skip the rAF loop entirely.
    const reduceMotion =
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uMouseReact: { value: reduceMotion ? 0 : 1 },
      },
    });
    ...
    const resize = () => {
      ...
      // The rAF loop is off under reduced motion, so a resize needs its own
      // repaint to keep the static frame in sync with the new canvas size.
